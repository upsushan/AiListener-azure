...
public class SwiftAzureSpeechRecognitionPlugin: NSObject, FlutterPlugin {
    ...
    var transcribe: SPXConversationTranscriber? = nil
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let diarization = args?["diarization"] as? String ?? ""
        ...
        if (call.method == "simpleVoice") {
            ...
            continuousStream(speechSubscriptionKey: speechSubscriptionKey, serviceRegion: serviceRegion, lang: lang, diarization:diarization)
            result(true)
        }
        ...
        let task = Task {
            ...
            if (Task.isCancelled) {
                ...
                if result.reason != SPXResultReason.recognizedSpeech {
                    ...
                    self.azureChannel.invokeMethod("speech.onFinalResponse", arguments: ["argument1": "", "argument2": ""])
                }
                else {
                    self.azureChannel.invokeMethod("speech.onFinalResponse", arguments: ["argument1": result.text, "argument2": ""])
                }
    ...
    private func continuousStream(speechSubscriptionKey : String, serviceRegion : String, lang: String, diarization: String) {
    if diarization == "yes" {

        if (continousListeningStarted) {
            print("Stopping continous recognition")
            do {
                try transcribe!.stopTranscribingAsync({ (started, err) in
            ...
            transcribe = try! SPXConversationTranscriber(speechConfiguration: speechConfig, audioConfiguration: audioConfig)


   try! transcribe!.addTranscribingEventHandler() {reco, evt in
                self.azureChannel.invokeMethod("speech.onSpeech", arguments: evt.result?.text)
              }

           try! transcribe!.addTranscribedEventHandler() {reco, evt in
                self.azureChannel.invokeMethod("speech.onFinalResponse",  arguments: ["argument1": evt.result?.text, "argument2": evt.result?.speakerId])
              }

                 try! transcribe!.addSessionStartedEventHandler() {_, evt in
                          print("session started")
                      }

                     try! transcribe!.addSessionStoppedEventHandler() {_, evt in
                          print("session stopped")
                      }

                     try! transcribe!.addCanceledEventHandler() {_, evt in
                          print("session canceled")
                      }

            try! transcribe!.startTranscribingAsync({ (started, err) in
 ...
 if (continousListeningStarted) {
            ...
            continousSpeechRecognizer!.addRecognizedEventHandler({reco, evt in
                ...
                self.azureChannel.invokeMethod("speech.onFinalResponse", arguments: ["argument1": res, "argument2":""])
            })
            print("Listening...")
