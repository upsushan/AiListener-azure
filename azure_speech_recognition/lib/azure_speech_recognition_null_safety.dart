...
typedef void MultiStringResultHandler(Map arguments);
...
class AzureSpeechRecognition {
  ...
  static String _diarization = "yes";
  ...
  AzureSpeechRecognition.changeLanguage(String lang){
    _lang = lang;
  }

  AzureSpeechRecognition.differentiateSpeakers(bool val){
    _diarization = val ? "yes" : "no";
  }

  StringResultHandler? exceptionHandler;
  StringResultHandler? recognitionResultHandler;
  MultiStringResultHandler? finalTranscriptionHandler;
  ...
  void setFinalTranscription(MultiStringResultHandler handler) =>
  ...
  static void continuousRecording() {
    if (_subKey != null && _region != null) {
      _channel.invokeMethod('continuousStream',
          {'language': _lang, 'subscriptionKey': _subKey, 'region': _region , 'diarization': _diarization});
    } else {
      throw "Error: SpeechRecognitionParameters not initialized correctly";
    }
  }
